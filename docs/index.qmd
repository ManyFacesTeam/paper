...
However, many studies tend to rely on a small number of those stimulus databases, which introduces potential problems. For example, as of September 2026 on Google Scholar, at least 4089 published papers cite the Karolinska Directed Emotional Faces [@lundqvist_karolinska_1998], 4592 cite the NimStim Set of Facial Expressions [@tottenham_nimstim_2009], 3406 cite the Radboud Faces Database  [@langner_presentation_2010], and 2867 cite the Chicago Face Database [@ma_chicago_2015]. Although citation counts are not a direct measure of the use of these databases, they nonetheless indicate their popularity. This frequent (over)use of the same sets of stimuli can compromise generalizability when various research questions are tested on only a limited sample of face images from specific backgrounds [@debruine_reproducible_2022; @yarkoni_generalizability_2022]. Participants, especially those participating online, may also become familiar with frequently used stimuli, potentially introducing biases to their responses.  
...
```{r}
lab_n <- data_models$lab_id |> unique() |> length()+1
...
A total of 211 models provided their images for use in Phase 2, following exclusions for poor image quality (as determined by the researchers collecting the images and the core team) and two model withdrawals. Of these, `r model_n` consented to have their images shared in a database. @tbl-images-per-type details the number of models for each kind of photo available in the database. Example images are shown in @fig-img-examples. Images are freely available for research use and can be accessed via UK Data Service ReShare: [**LINK**]<!--LISA TO ADD LINK-->. Note that researchers must agree to our terms of use to access the database, in line with ethical constraints (see <https://doi.org/10.5281/zenodo.22707036>).
...
# *** LISA: extract these data from elsewhere, as data_models only had front-facing image info (now removed from that file ***)
...
#*** LISA: update image labels to be grammatically consistent
...
These processed images are available alongside the full set of RAW images in the image database.
...
#| fig-cap-location: top
#| fig-alt: "Example face images for the 8 types of stimuli: unstandardized neutral, standardized neutral, angry, disgusted, fearful, happy, sad, and surprised"

# ***LISA: change labels in image to be grammatically consistent ***
...
We present here only the results for ratings of images of models who agreed to be included in the database, rather than all images raters saw.
...
In this section, we report the descriptive statistics for trait ratings of the standardized and unstandardized neutral images, the emotion categorizations and emotion intensity ratings of the standardized emotional images, and the perceived ages of the models (standardized neutral images).
  
##### Standardized and unstandardized neutral trait ratings

For standardized neutral faces, trustworthiness, dominance, and memorability ratings were approximately normally distributed and showed similar patterns of central tendency and variance. In contrast, attractiveness ratings were somewhat right-skewed, whereas gender-typicality ratings were left-skewed (see @fig-neu-hist). For unstandardized neutral faces, patterns for attractiveness and dominance were similar to those for standardized neutral faces, but trustworthiness ratings showed slight left skew.
...
plot_unstd <- unstd_prop |>
  ...
  geom_text(data = unstd_MSD,
            mapping = aes(x = 6, y = .3, label = label)) +
  scale_x_continuous(breaks = 1:7) +
  labs(title = "B Unstandardized neutral trait ratings",
       x = "Rating",
       y = "Proportion") +
  theme_minimal() +
  theme(strip.text = element_text(size = 11, face = "bold"),
...
Next, we explored raters’ emotion categorizations and perceived emotion intensity expressed by our models. Raters’ categorizations generally aligned with the models’ expression of emotion, with the greatest alignment observed for expression of happiness, and the least for fear, with fearful faces most often perceived as expressing surprise rather than fear (see @fig-emo-freq, @tbl-emo-desc in SM). 
...
data_exp_rename |>
  ...
  flextable::width(j = 1, 2)
```
*Note*. The emotion categorization task was broken into 6 counterbalanced versions.  
...
data_quest_rename |> 
  ...
  flextable::width(j = 1, 2)
```
*Note*. Where rows list more than one country, *each* country in that row has the specified number of participants.  
