...
(results_trait_std$plot + ggtitle("Standardised trait ratings")) /
  (results_trait_unstd$plot + ggtitle("Unstandardised trait ratings")) /
...
#| warning: false
#| message: false
alpha_omega_table <- data_trait_std |> 
  group_by(exp) |> 
...
To further check inter-rater agreement, we used both Cronbach’s alpha (α) and McDonald’s omega total (ωt; see @tbl-std-neu-alpha-omega). For four of the five traits, both α and ωt exceeded 0.90, indicating excellent inter-rater agreement. For the fifth trait (memorability), α was lower (`r alpha_omega_table |> filter(experiment == "memorable") |> pull(alpha)`), suggesting reduced inter-rater agreement. ωt for memorability was substantially higher (`r alpha_omega_table |> filter(experiment == "memorable") |> pull(omega_t)`), suggesting better agreement under the assumption that raters varied in how consistently they judged memorability, a pattern consistent with the lower ICC observed for this trait.
...
As with standardized trait ratings, we calculated intraclass correlation coefficients for ratings of unstandardized images. The number of raters ranged from  `r icc_table_unstd |> pull(n_raters) |> min()` to `r icc_table_unstd |> pull(n_raters) |> max()`. Average inter-rater reliability was excellent for all three traits (see @tbl-unstd-neu-icc). This was further supported by Cronbach's α ranging from `r min(alpha_omega_table_unstd$alpha)` to `r max(alpha_omega_table_unstd$alpha)` and McDonald's ωt ranging from `r min(alpha_omega_table_unstd$omega_t)` to `r max(alpha_omega_table_unstd$omega_t)` (see @tbl-unstd-neu-alpha-omega).
...
emo_alpha_omega_table <- data_emo_int |> 
  group_by(exp) |> 
  group_map(calc_alpha_omega) |> 
  bind_rows() |> 
  mutate(across(where(is.numeric), \(x) round(x, 2)))
...
Next, we calculated intraclass correlation coefficients, Cronbach's α, and McDonald's ωt for emotion intensity ratings. The number of raters ranged from `r icc_table_emo |> pull(n_raters) |> min()` to `r icc_table_emo |> pull(n_raters) |> max()`. The average reliability across raters was excellent for all emotions, with ICC(2,k) ranging from `r min(icc_table_emo[["ICC(2,k)"]])` to `r max(icc_table_emo[["ICC(2,k)"]])`, Cronbach’s α ranging from `r min(emo_alpha_omega_table$alpha)` to `r max(emo_alpha_omega_table$alpha)`, and  McDonald's ωt ranging from `r min(emo_alpha_omega_table$omega_t)` to `r max(emo_alpha_omega_table$omega_t)` (see @tbl-emo-icc and @tbl-emo-alpha-omega).
