...
For gender and race/ethnicity, words presented in languages other than English were recoded to be presented in English (e.g., “mulher” to “female”, “preta” to “Black”). We then classified self-described race/ethnicity into one of seven categories (“White” (n = `r eth_n$White`), “Black” (n = `r eth_n$Black`), “Asian” (n = `r eth_n$Asian`), “Indigenous” (n = `r eth_n$Indigenous`), “MENA” (n = `r eth_n$MENA`)<!-- this code leaves a blank--> (Middle Eastern or North African), “Latine” (n = `r eth_n$Latine`), or “Mixed” (n = `r eth_n$Mixed`)), where possible. Descriptions that could not be clearly sorted into these categories were given “Ambiguous label” (n = `r eth_n[["Ambiguous label"]]`)<!-- this code leaves a blank--> and non-entries were recoded as NA (n = `r eth_n[["NA"]]`). 
...
We recruited fluent English-speaking raters through Prolific <!--add prolific inclusion/exclusion criteria-->. We collected all data in May 2025^[This was slightly later than preregistered, due to pretesting and study setup delays. Note that we also planned to collect perceived race/ethnicity data, but decided against this, due to the regional specificity of race/ethnicity labels.].
...
s_n_min <- min(icc_table$N)
s_n_max <- max(icc_table$N)
```

In the first step, we calculated intraclass correlation coefficients [ICC(2,k)] for ratings of standardized images. The number of raters ranged from `r s_n_min` to `r s_n_max` for each trait. The average reliability across raters was excellent for ratings of attractiveness, dominance, trustworthiness, and gender-typicality, but poorer for ratings of memorability (see @tbl-std-neu-icc).

```{r}
#| label: tbl-std-neu-icc
#| tbl-cap: Intraclass correlation coefficients for ratings of standardized neutral faces
...
mem <- filter(alpha_omega_table, Rating == "memorable")
a_mem <- mem$alpha |> format(digits = 2)
o_mem <- mem$omega |> format(digits = 2)
``` 

To further check inter-rater agreement, we used both Cronbach’s alpha (α) and McDonald’s omega total (ωt; see @tbl-std-neu-alpha-omega). For four of the five traits, both α and ωt exceeded 0.90, indicating excellent inter-rater agreement. For the fifth trait (memorability), α was lower (`r a_mem`), suggesting reduced inter-rater agreement. ωt for memorability was substantially higher (`r o_mem`), suggesting better agreement under the assumption that raters varied in how consistently they judged memorability, a pattern consistent with the lower ICC observed for this trait.

```{r}
#| label: tbl-std-neu-alpha-omega
#| tbl-cap: Cronbach’s alpha and McDonald’s omega total for ratings of standardized neutral faces.
...
u_n_min <- min(icc_table_unstd$N)
u_n_max <- max(icc_table_unstd$N)

alpha_omega_table_unstd <- data_trait_unstd |> 
  group_by(exp) |> 
...
u_a_min <- min(alpha_omega_table_unstd$alpha) |> format(digits = 2)
u_a_max <- max(alpha_omega_table_unstd$alpha) |> format(digits = 2)
u_o_min <- min(alpha_omega_table_unstd$omega) |> format(digits = 2)
u_o_max <- max(alpha_omega_table_unstd$omega) |> format(digits = 2)
``` 

As with standardized trait ratings, we calculated intraclass correlation coefficients for ratings of unstandardized images. The number of raters ranged from  `r u_n_min` to `r u_n_max`. Average inter-rater reliability was excellent for all three traits (see @tbl-unstd-neu-icc). This was further supported by Cronbach's α ranging from `r u_a_min` to `r u_a_max` and McDonald's ωt ranging from `r u_o_min` to `r u_o_max` (see @tbl-unstd-neu-alpha-omega).


```{r}
#| label: tbl-unstd-neu-icc
...
```{r}
#| label: tbl-unstd-neu-alpha-omega
#| tbl-cap: Cronbach’s alpha and McDonald’s omega total for ratings of unstandardized neutral faces.

alpha_omega_table_unstd |>
  flextable() |>
  colformat_double(digits = 2) |>
  flextable::width(j = 1, 2)
```
...
emo_n_min <- min(icc_table_emo$N)
emo_n_max <- max(icc_table_emo$N)

emo_icc_min <- min(icc_table_emo$ICC) |> format(digits = 2)
emo_icc_max <- max(icc_table_emo$ICC) |> format(digits = 2)

alpha_omega_table_emo <- data_emo_int |> 
  group_by(exp) |> 
  group_map(calc_alpha_omega) |> 
  bind_rows() |>
  rename(Rating = experiment,
...
emo_a_min <- min(alpha_omega_table_emo$alpha) |> format(digits = 2)
emo_a_max <- max(alpha_omega_table_emo$alpha) |> format(digits = 2)
emo_o_min <- min(alpha_omega_table_emo$omega) |> format(digits = 2)
emo_o_max <- max(alpha_omega_table_emo$omega) |> format(digits = 2)
```

Next, we calculated intraclass correlation coefficients, Cronbach's α, and McDonald's ωt for emotion intensity ratings. The number of raters ranged from `r emo_n_min` to `r emo_n_max`. The average reliability across raters was excellent for all emotions, with ICC(2,k) ranging from `r emo_icc_min` to `r emo_icc_max`, Cronbach’s α ranging from `r emo_a_min` to `r `, and  McDonald's ωt ranging from `r emo_o_min` to `r emo_o_max` (see @tbl-emo-icc and @tbl-emo-alpha-omega).


```{r}
#| label: tbl-emo-icc
#| tbl-cap: Intraclass correlation coefficients for ratings of emotional faces’ emotion intensity
...
```{r}
#| label: tbl-emo-alpha-omega
#| tbl-cap: Cronbach’s alpha and McDonald’s omega total for ratings of emotional faces’ emotion intensity

alpha_omega_table_emo |>
  flextable() |>
  colformat_double(digits = 2) |>
  flextable::width(j = 1, 2)
```
...
summary_ci_traits |> 
  mutate(exp = factor(exp, exp_levels_neutral)) |>
...
summary_ci_traits_unstd |> 
  mutate(exp = factor(exp, exp_levels_neutral_unstd)) |>
  ggplot(aes(x = sample_size)) +
  geom_ribbon(aes(ymin = ll, ymax = ul, fill = exp),
              alpha = 0.5) +
...
The distribution of raters’ perceptions of models’ ages is shown in @fig-age-hist. 26-30 years was the most-chosen age across raters and models. The correlation between models’ mode perceived age and actual age appears in @fig-age-corr.
