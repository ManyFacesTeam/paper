...
data_models <- model_raw |>
  ...
  mutate(ethnicity_rec = factor(ethnicity_rec,
                                levels = c("White", "Black", "Asian", "Indigenous", "Middle Eastern or North African (MENA)", "Latine", "Mixed", "Ambiguous label"))) |>
  mutate(across(c(age, weight, height), as.numeric)) |>
  # recode height and weight
  # one person reported weight of 37 kg at 163 cm -- recode as NA? Would be a BMI of 13.9
  mutate(height_cm = case_when(height_units == "cm" & height < 100 ~ height * 2.54, # likely mislabelled inches
                               height_units == "in" & height > 100 ~ height, # likely mislabelled cms
                               height_units == "in" ~ height * 2.54,
                               height_units == "cm" ~ height,
                               .default = NA_real_),
         weight_kg = case_when(as.numeric(weight) == 0 ~ NA_real_,
                               ...
                               weight_units == "lb" & weight < 80 ~ weight,            # likely mislabelled kg
                               weight_units == "lb" ~ weight / 2.20462,
                               weight_units == "kg" ~ weight,
                               weight == 0 ~ NA,
                               .default = NA_real_)) |>
...
data_exp <- exp_long |>
  ...
  mutate(exp = factor(exp, levels = c("attractive",
                                      "dominant",
                                      "trustworthy",
                                      "attractive (unstd)",
                                      "dominant (unstd)",
